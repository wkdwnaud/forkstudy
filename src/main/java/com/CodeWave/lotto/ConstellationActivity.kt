    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        goResultButton.setOnClickListener {
            ...
            intent.putIntegerArrayListExtra(
                "result",
                ArrayList(LottoNumberMaker.getLottoNumberFromHash(Star_textview.text.toString()))
            )
            intent.putExtra("constellation", Star_textview.text.toString())
            startActivity(Intent(intent))
        }

        //datePicker의 월일 정보에 맞게 별자리의 텍스트를 변경한다.
        Star_textview.text = makeConstellationString(datePicker.month, datePicker.dayOfMonth)
